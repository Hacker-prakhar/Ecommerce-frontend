...
export const getAllCategoriesDashboard = (page=0) => async (dispatch) => {
  dispatch({ type: "CATEGORY_LOADER" });
  try {
    const { data } = await axios.get("http://localhost:8000/api/public/categories", {
...
export const updateCategoryDashboardAction =
  ...
  async (dispatch, getState) => {
    try {
      ...
      await api.put(`/public/categories/${categoryID}`, sendData);
...
export const deleteCategoryDashboardAction =
  (setOpen, categoryID, toast) => async (dispatch, getState) => {
    try {
      ...
      console.log(err);
      toast.error(err?.response?.data?.message || "Failed to delete category whose products are present in order");
      dispatch({
        type: "IS_ERROR",
        payload: err?.response?.data?.message || "Internal Server Error",
      });
