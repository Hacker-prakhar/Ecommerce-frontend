...
export const fetchProducts = (queryString) => async (dispatch) => {
    try {
        ...
        const { data } = await api.get(
            `/public/products${queryString ? `?${queryString}` : ""}`
        );
...
export const increaseCartQuantity =
    ...
    (dispatch, getState) => {
        ...
        const isQuantityExist =
            getProduct.quantity >= currentQuantity + 1;
...
export const deleteCategoryDashboardAction =
    ...
    async (dispatch, getState) => {
        ...
        try {
            ...
            toast.error(
                err?.response?.data?.message ||
                "Failed to delete category whose products are present in order"
            );
...
export const getAllSellersDashboard =
    ...
    async (dispatch, getState) => {
        ...
        try {
            ...
            const { data } = await api.get(
                `/auth/sellers${queryString ? `?${queryString}` : ""}`
            );
